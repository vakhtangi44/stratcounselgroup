...
export default function AboutValuesReorderList({ initialItems }: { initialItems: AboutValue[] }) {
  ...
  async function saveOrder() {
    setSaving(true)
    await fetch('/api/admin/about-values/reorder', {
      method: 'PUT',
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="font-heading text-2xl text-dark">Our Values (About Page)</h1>
        <div className="flex items-center gap-3">
          ...
          <Link href="/admin/about-values/new" className="bg-gold text-white px-4 py-2 rounded text-sm hover:bg-gold-dark">
            + New Value
          </Link>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          ...
          <tbody className="divide-y divide-gray-100">
            {items.map((item, i) => (
              ...
              >
                ...
                <td className="px-4 py-3">
                  <Link href={`/admin/about-values/${item.id}/edit`} className="text-gold hover:underline">
                    Edit
                  </Link>
                </td>
              </tr>
        ...
        {items.length === 0 && (
          <p className="text-center text-secondary py-8">No values yet.</p>
        )}
      </div>
