            ...
            {advantages.map((v, i) => (
              <ScrollReveal key={v.id} delay={i * 100}>
                <div className="flex items-start gap-4 p-6 bg-cream border border-gray-100 hover:border-gold/30 transition-all duration-500 group">
                  ...
                  <p className="text-dark text-base leading-relaxed">{isKa ? v.titleKa : v.titleEn}</p>
                </div>
              </ScrollReveal>
            ))}
          </div>
            ...
            {[
              {
                ...
                titleKa: 'ექსპერტიზა', titleEn: 'Expertise',
                descKa: 'წლების გამოცდილება სახელმწიფო და კერძო სექტორში',
                descEn: 'Years of experience across public and private sectors',
              },
              {
                icon: (
                  <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                ...
                titleKa: 'სტრატეგია', titleEn: 'Strategy',
                descKa: 'გამჭვირვალე, შედეგზე ორიენტირებული მიდგომა',
                descEn: 'Transparent, results-driven approach to every matter',
              },
              {
                icon: (
                  <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                ...
                titleKa: 'პარტნიორობა', titleEn: 'Partnership',
                descKa: 'გრძელვადიანი ურთიერთობა კლიენტებთან',
                descEn: 'Long-term relationships built on trust and results',
              },
            ].map((v, i) => (
              <ScrollReveal key={v.titleEn} delay={i * 150}>
                <div className="text-center p-8 bg-white border border-gray-100 hover:border-gold/30 transition-all duration-500 group hover:shadow-lg hover:shadow-gold/5">
                  ...
                  <h3 className="font-heading text-xl text-dark mb-3">{isKa ? v.titleKa : v.titleEn}</h3>
                  <p className="text-secondary text-sm font-light leading-relaxed">{isKa ? v.descKa : v.descEn}</p>
                </div>
              </ScrollReveal>
            ))}
          </div>
        ...
        <div className="relative z-10">
          <ScrollReveal>
            ...
            <h2 className="font-heading text-3xl md:text-4xl mb-6 tracking-[-0.01em]">
              {isKa ? 'კონსულტაციისთვის დაგვიკავშირდით' : 'Ready to work with us?'}
            </h2>
            <p className="text-white/40 mb-10 max-w-lg mx-auto font-light">
              {isKa
                ? 'დაგვიკავშირდით და მოგაწვდით კვალიფიციურ იურიდიულ კონსულტაციას.'
                : 'Contact us for qualified legal counsel tailored to your needs.'}
            </p>
            <Link
              href={`${prefix}/contact`}
