...
export default async function Hero({ locale, strings }: { locale: string; strings: HeroStrings }) {
  ...
  const headline = rawHeadline.replace(/\.\s?/g, '<span style="color:#d88551;font-weight:700;margin:0 0.05em;font-size:0.8em">-</span>')
  const hasHtml = true
  const words: string[] = []
  ...
  const headingSize =
    locale === 'en'
      ...
      : 'text-[0.70rem] sm:text-[1.04rem] md:text-[1.74rem] lg:text-[2.09rem]'
