...
function CaseCard({ text, icon }: { text: string; icon: string }) {
  return (
    <div tabIndex={0} className="relative h-full p-5 sm:p-8 bg-white border border-gray-100 transition-all duration-700 group text-center outline-none card-hover focus:scale-[1.05]">
      ...
      <p className="leading-relaxed font-normal text-justify group-hover:text-dark group-focus:text-dark transition-colors duration-500" style={{ fontSize: 'var(--typo-cardBody-size, 0.875rem)', color: 'var(--typo-cardBody-color, #1C122C)', fontFamily: 'var(--typo-cardBody-font)' }}>
        {text}
      </p>
    </div>
  )
