...
function CaseCard({ text, iconKey }: { text: string; iconKey: string }) {
  return (
...
export default function SuccessfulCases({ locale }: Props) {
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 max-w-5xl mx-auto">
            {featured.map((text, i) => (
              <ScrollReveal key={i} delay={i * 100}>
                <CaseCard text={text} iconKey={CASE_ICONS[i]} />
              </ScrollReveal>
            ))}
          </div>
        </div>
      ...
      {/* Our Experience — remaining cases */}
      {rest.length > 0 && (
        <section className="py-20 md:py-28 bg-cream">
          <div className="container mx-auto px-4 lg:px-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
              {rest.map((text, i) => {
                const originalIndex = i + FEATURED_COUNT
                return (
                  <ScrollReveal key={originalIndex} delay={(i % 6) * 100}>
                    <CaseCard text={text} iconKey={CASE_ICONS[originalIndex]} />
                  </ScrollReveal>
                )
              })}
            </div>
